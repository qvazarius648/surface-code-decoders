...
Provides the rotated and unrotated planar surface code as memory experiments,
with several noise models commonly used in the QEC literature. Circuits are
generated by stim and wrapped in a SurfaceCodeCircuit dataclass that carries
the configuration alongside the circuit object.
 
The standard benchmark is the rotated surface code memory experiment with
uniform depolarizing noise, as used in Fowler et al. (arXiv:1208.0928).
"""
...
"""
Surface code variants supported by stim.Circuit.generated().
 
rotated_memory_x / _z:
    The rotated (tilted) surface code. Uses d² data qubits and d²-1 ancilla
    qubits. Roughly half the qubit overhead of the unrotated version at the
    same distance. Standard choice for hardware experiments and benchmarks.
    _x and _z refer to which logical basis is stored in memory.
 
unrotated_memory_x / _z:
    The original (CSS) surface code layout. Uses 2d²-2d+1 data qubits.
    Higher qubit overhead but sometimes used for theoretical analysis.
"""
...
class NoiseModel:
    """
    Noise parameters for surface code circuit generation.
 
    Three independent noise channels are supported, corresponding to stim's
    circuit-level noise parameters:
 
    Attributes:
        depolarizing: Symmetric depolarizing probability applied to each data
                      qubit before every syndrome extraction round.
                      (stim: before_round_data_depolarization)
        measurement:  Bit-flip probability applied to each ancilla measurement
                      outcome. Models readout errors.
                      (stim: before_measure_flip_probability)
        reset:        Bit-flip probability applied after each qubit reset.
                      Models leakage or imperfect initialization.
                      (stim: after_reset_flip_probability)
    """
